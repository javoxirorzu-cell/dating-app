...
const defaultSiteConfig = {
  brandName: "Luvora",
  headerTagline: "Swipe like Tinder. Feel like Luvora.",
  homeHeroTitle: "Swipe smarter. Match faster.",
  homeHeroSubtitle: "A crisp Tinder-style deck with your best picks. Manage everything from one dashboard.",
  accent: "#ff6b81",
  accent2: "#ffa351",
  bg: "#0c0f16"
};
...
function getSiteConfig() {
  return getJSON("luv_site_config", defaultSiteConfig);
}
...
function applySiteConfig() {
  ...
  const map = [
    ...
    ["homeHeroSubtitle", config.homeHeroSubtitle]
  ];
  ...
  document.documentElement.style.setProperty("--accent", config.accent || defaultSiteConfig.accent);
  document.documentElement.style.setProperty("--accent-2", config.accent2 || defaultSiteConfig.accent2);
  document.documentElement.style.setProperty("--bg", config.bg || defaultSiteConfig.bg);
}

function initEditorForm() {
  const config = getSiteConfig();
  const fields = {
    cfgBrand: config.brandName,
    cfgTagline: config.headerTagline,
    cfgHeroTitle: config.homeHeroTitle,
    cfgHeroSub: config.homeHeroSubtitle,
    cfgAccent: config.accent,
    cfgAccent2: config.accent2,
    cfgBg: config.bg
  };
...
function saveSiteConfig() {
  const config = {
    brandName: document.getElementById("cfgBrand")?.value || defaultSiteConfig.brandName,
    headerTagline: document.getElementById("cfgTagline")?.value || defaultSiteConfig.headerTagline,
    homeHeroTitle: document.getElementById("cfgHeroTitle")?.value || defaultSiteConfig.homeHeroTitle,
    homeHeroSubtitle: document.getElementById("cfgHeroSub")?.value || defaultSiteConfig.homeHeroSubtitle,
    accent: document.getElementById("cfgAccent")?.value || defaultSiteConfig.accent,
    accent2: document.getElementById("cfgAccent2")?.value || defaultSiteConfig.accent2,
    bg: document.getElementById("cfgBg")?.value || defaultSiteConfig.bg
  };

  setJSON("luv_site_config", config);
  applySiteConfig();
  showToast("Website design saved");
}

function resetSiteConfig() {
  setJSON("luv_site_config", defaultSiteConfig);
  initEditorForm();
  applySiteConfig();
  showToast("Default design restored");
}

window.saveSiteConfig = saveSiteConfig;
window.resetSiteConfig = resetSiteConfig;
window.initEditorForm = initEditorForm;
